	...
	//绘制月历体——————————————————————————————————————————
	 ...
	 $("<li><span></span></li>").appendTo(".month-body").addClass("month-cell"); 
	  ...
	  if(firstDay<0){                       //如果月初为负，加七循环
  		...
  		String s = (String)session.getAttribute("currenttea");
  			int tmp=t.getCurrentTea(s);
  		%>
	  var f = firstDay;
	  for(var j=1;j<=monthsNum[m];j++){
		  $("li.month-cell span").eq(f).text(j).parent().addClass("pink");
			$("li.month-cell span").eq(f).html("<a href=../jdbc/viewbyteacher?date2="+FullYear+"-"+month+"-"+(date+f-27)+"&"+
					"teacherId="+<%=tmp%>+" target='showframe' style='text-decoration:none'>"+j+"</a>");
		  f++; 
	  }
